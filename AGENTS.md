# bricksfit MVP：项目约定与维护指南

本文件适用于整个 `setfit-mvp` 目录。后续代理或开发者修改本项目时，应先阅读本文件，再检查与任务直接相关的源码。

## 1. 项目目标与当前状态

bricksfit 是一个面向搜索流量和展示空间规划的独立 LEGO 套装尺寸网站。核心用户问题是：
...
- 页面采用紧凑但可读的内容密度：展示型 H1 不应挤占大半个首屏，桌面套装网格优先四列，区块、卡片和计算器避免无意义的大面积留白；窄屏标题与导航必须在 680–780px 过渡区间内保持可用，不能沿用桌面巨型字号。
...
- 计算器的摆放方向必须由用户明确切换“常规摆放”或“水平旋转 90°”；切换时宽和深互换，高度保持不变，3D 预览、逐轴对比和最终结论必须使用同一个选中方向，不能偷偷自动替用户选择方向。
- 3D 预览中的套装图片需要随摆放方向一起变化：常规方向贴在套装长方体正面，水平旋转 90° 后转到可见侧面，不能只交换长宽而把图片留在原正面。
- 3D 预览中的套装外包络默认使用中性的蓝灰色；红色只用于当前摆放方向确实放不下的错误状态，不能让正常或尚未失败的模型看起来像报错。
- 计算器的套装搜索选择器应在当前选中项和展开的结果列表中显示本地套装缩略图；缺图记录使用套装编号占位，不能产生破图。
- 柜架预设最左侧固定为“自定义尺寸”，选中后聚焦并引导用户在下方输入内部净宽、净深和净高。常见 IKEA、MUJI 家具排在大型展示柜之前，大型展示柜放在最后。预设卡片不得链接品牌或零售商官网，也不要写成推荐或购买广告。
...
- LEGO® 商标免责声明和 Brickset 图片署名必须保留。bricksfit 不得表现为 LEGO Group 或 Brickset 的官方产品。
...
- 顶栏语言切换器使用与 CM/IN 单位控件一致的自定义胶囊视觉，不要退回浏览器原生 `<select>`；菜单必须支持键盘方向键、Esc 关闭、当前语言状态和点击外部关闭，并继续由 `siteConfig.locales` 驱动。
...
- 计算器同时计算两个方向，但界面只以用户当前选中的摆放方向生成逐轴对比和结论；若另一个方向能放入，可以提示用户切换，不能把另一个方向的结果冒充为当前方向结果；
...
- `data/furniture-presets.ts`：柜架预设的内部规划尺寸、显示顺序和本地图片；前台预设不包含品牌官网链接；
...
- `components/CabinetFitGuide.tsx`：旧 BILLY 指南 URL 下的可交互柜架尺寸选择和整表重算；
- `components/GuideSetIdentity.tsx`：两个指南表格共用的套装缩略图与详情入口；
...
`/guides/sets-for-ikea-billy` 是已有 SEO URL，继续保持路径稳定，但页面内容已经泛化为可选择柜架预设或自定义尺寸的适配表。不要把正文退回为固定 BILLY 尺寸。这个指南和 `/guides/large-brick-sets-under-30cm-deep` 的套装列表都必须在名称前显示本地套装缩略图，并保留缺图占位状态。
